...
class FmtTradeFilesFromDifferentBrokers:
    ...
    def fmt_jpm(self, fpath):
        df_csv = pd.read_csv(fpath, converters={'Gross Price': lambda x: round(float(x), 4)})
        df_ret = pd.DataFrame(data=[], columns=self.COLS)
        ...
        df_ret["Avg Price"] = df_csv["Gross Price"]
        df_ret["FillPrice"] = df_csv["Gross Price"]
        ...
        dict_suffix2connect_qffi = {'.ZK': 'connect', '.SH': 'connect', '.SZ': 'qfii', '.SS': 'qfii'}
        ...
        df_ret["Commission"] = abs(round(df_csv["Initial Quantity"] * df_csv["Gross Price"] * df_csv['fee'] / 1e4, 4)).round(decimals=4)
...
if __name__ == '__main__':
    task = FmtTradeFilesFromDifferentBrokers('20200817', ['jpm'])
    # task = FmtTradeFilesFromDifferentBrokers('20200817', ['ms','cs'])
    task.run()
